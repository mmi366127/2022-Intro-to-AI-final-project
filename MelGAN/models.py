...
from tensorflow.keras.layers import Flatten, Dense, LeakyReLU, ReLU, Conv2D, BatchNormalization, UpSampling2D, Concatenate, Input
...
def build(hop, shape, vec_len):
    # tensorflow put channel dim at last dim.
    model_S = build_siamese((hop, shape, 1), vec_len)
    model_G = build_generator((hop, shape, 1))
    model_D = build_critic((hop, shape * 3, 1))
    return model_S, model_G, model_D

def load(path, hop, shape, vec_len):
    # tensorflow put channel dim at last dim.
    model_S = build_siamese((hop, shape, 1), vec_len)
    model_G = build_generator((hop, shape, 1))
    model_D = build_critic((hop, shape * 3, 1))
    model_S.load_weights(os.path.join(path, 'siam.h5'))
    model_G.load_weights(os.path.join(path, 'gen.h5'))
    model_D.load_weights(os.path.join(path, 'critic.h5'))
    return model_S, model_G, model_D
